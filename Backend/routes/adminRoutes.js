  ...
  // Add this endpoint to get all unread counts for admin
router.get('/unread-counts/:adminId', authenticateToken, authorizeAdmin, async (req, res) => {
  try {
    ...
    const unreadMessages = await Chat.aggregate([
      {
        $match: {
          receiverId: adminId,
          read: false,
          senderRole: { $ne: 'admin' } // Only count messages FROM users (not admin's own messages)
        }
      },
      {
        $group: {
    ...
    unreadMessages.forEach(item => {
      ...
      unreadCounts[item._id + '_lastMsg'] = item.lastMessage;
      unreadCounts[item._id + '_lastMsgTime'] = item.lastMessageTime;
    });
    
    console.log('Unread counts fetched for admin:', Object.keys(unreadCounts).length);
